...
import {getRecordsTechnicalByID,updateRecordsTechnical, addRecordsTechnical, uploadRecordsTechnical } from "./API_Routes";
...
export default function TechnicalCompetitions() {
  ...
  const fetchRecord = async (tableName, table_id) => {
    try {
      ...
      console.log("id: ", id);
      if (table_id !== null) {
        const recordTechnicalURL = getRecordsTechnicalByID(
          table_id,
          currentUser?.Username
        );
        console.log("in Technical: ", recordTechnicalURL);
        const response = await axios.get(recordTechnicalURL);
        console.log("record response in Technical: ", response.data.data[0]);
        setFormData({
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      await axios.put(
        `${updateRecordsTechnical}?username=${currentUser?.Username}&T_ID=${id}`,
