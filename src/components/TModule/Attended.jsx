...
import { getRecordsAttendedByID,updateRecordsAttended,addRecordsAttended, uploadRecordsAttended } from "./API_Routes";
...
export default function Attended() {
  ...
  const fetchRecord = async (tableName, table_id) => {
    try {
      ...
      console.log("id: ", id);
      if (table_id !== null) {
        const recordAttendedURL = getRecordsAttendedByID(
          table_id,
          currentUser?.Username
        );
        console.log("in attended: ", recordAttendedURL);
        const response = await axios.get(recordAttendedURL);
        console.log("record response in attended: ", response.data.data[0]);
        setFormData({
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      await axios.put(
        `${updateRecordsAttended}?username=${currentUser?.Username}&T_ID=${id}`,
