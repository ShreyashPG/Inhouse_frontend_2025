...
import {
  addRecordsConsultancy,
  uploadRecordsConsultancy,
  getRecordConsultancyByID,
...
export default function ConsultancyReport() {
  ...
  const fetchRecord = async (table_id) => {
    try {
      if (table_id !== null) {
        const recordURL = getRecordConsultancyByID(table_id, currentUser?.Username);
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      // Send a PUT request to the updateRecordsConsultancy API endpoint
      await axios.put(
        `${updateRecordsConsultancy}?username=${currentUser?.Username}&T_ID=${id}`,
