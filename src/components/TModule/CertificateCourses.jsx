...
import { addRecordsCertificate, uploadRecordsCertificate, getRecordsCertificateByID, updateRecordsCertificate } from "./API_Routes";
...
export default function CertificateCourses() {
  ...
  const fetchRecord = async (tableName, table_id) => {
    try {
      ...
      console.log("id: ", id);
      if (table_id !== null) {
        const recordCertificateURL = getRecordsCertificateByID(
          table_id,
          currentUser?.Username
        );
        console.log("in certificate: ", recordCertificateURL);
        const response = await axios.get(recordCertificateURL);
        console.log("record response in certificate: ", response.data.data[0]);
        setFormData({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Send a POST request to the addRecordsCertificate API endpoint
 ...
 const handleUpdate = async (e) => {
  ...
  try {
    ...
    await axios.put(
      `${updateRecordsCertificate}?username=${currentUser?.Username}&T_ID=${id}`,
