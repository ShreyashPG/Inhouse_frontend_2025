...
export default function FacultyAchievements() {
  ...
  const fetchRecord = async (tableName, table_id) => {
    try {
      ...
      console.log("id: ", id);
      if (table_id !== null) {
        const recordAchievementsURL = getRecordsAchievementsByID(
          table_id,
          currentUser?.Username
        );
        console.log("in achievements: ", recordAchievementsURL);
        const response = await axios.get(recordAchievementsURL);
        console.log("record response in achievements: ", response.data.data[0]);
        setFormData({
  ...
  const handleSubmit = async (e) => {
   ...
   const handleUpdate = async (e) => {
    ...
    try {
      ...
      await axios.put(
        `${updateRecordsAchievements}?username=${currentUser?.Username}&T_ID=${id}`,
