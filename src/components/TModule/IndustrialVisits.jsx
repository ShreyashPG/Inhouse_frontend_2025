...
import { getRecordsIndustrialByID,updateRecordsIndustrial,addRecordsIndustrial, uploadRecordsIndustrial } from "./API_Routes";
...
export default function IndustrialVisit() {
  ...
  const fetchRecord = async (tableName, table_id) => {
    try {
      ...
      console.log("id: ", id);
      if (table_id !== null) {
        const recordIndustrialURL = getRecordsIndustrialByID(
          table_id,
          currentUser?.Username
        );
        console.log("in Industrial: ", recordIndustrialURL);
        const response = await axios.get(recordIndustrialURL);
        console.log("record response in Industrial: ", response.data.data[0]);
        setFormData({
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      await axios.put(
        `${updateRecordsIndustrial}?username=${currentUser?.Username}&T_ID=${id}`,
