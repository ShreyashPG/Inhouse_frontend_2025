...
import {getRecordGrantsByID,updateRecordsGrants, addRecordsGrants, uploadRecordsGrants } from "./API_Routes";
...
export default function Grants() {
  ...
  const fetchRecord = async (tableName, table_id) => {
    try {
      ...
      console.log("id: ", id);
      if (table_id !== null) {
        const recordGrantsURL = getRecordGrantsByID(
          table_id,
          currentUser?.Username
        );
        console.log("in grant: ", recordGrantsURL);
        const response = await axios.get(recordGrantsURL);
        console.log("record response in grant: ", response.data.data[0]);
        setFormData({
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      await axios.put(
        `${updateRecordsGrants}?username=${currentUser?.Username}&T_ID=${id}`,
