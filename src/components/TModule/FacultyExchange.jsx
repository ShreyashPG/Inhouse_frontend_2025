...
import { getRecordsMousByID,updateRecordsMous,addRecordsMous, uploadRecordsMous } from "./API_Routes";
...
export default function FacultyExchange() {
  ...
  const fetchRecord = async (tableName, table_id) => {
    try {
      ...
      console.log("id: ", id);
      if (table_id !== null) {
        const recordMousURL = getRecordsMousByID(
          table_id,
          currentUser?.Username
        );
        console.log("in Mous: ", recordMousURL);
        const response = await axios.get(recordMousURL);
        console.log("record response in Mous: ", response.data.data[0]);
        setFormData({
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      await axios.put(
        `${updateRecordsMous}?username=${currentUser?.Username}&T_ID=${id}`,
